...
from .utils.data import tensors_to_pil, one_hot_to_idx
...
def acedm_sampler(
    ...
    step_indices = torch.arange(num_steps, dtype=dtype, device=x.img.device)
    t_steps = (
        sigma_max ** (1 / rho)
        + step_indices
        / (num_steps - 1)
...
def generate_images(
    ...
    noise = torch.randn((num_images, 3, *img_size)).to(get_device())
