...
class PatchNeighborhoodDataset(Dataset):
    ...
    def __init__(
        ...
        folder_path,
        patch_size=(64, 64),
        form_transform=PairedFormalAugmentation,
        color_transform=ColorAugmentation,
        seed=42,
        preprocess=True,
        blank_ctl_p=0.3,
        copy_to=None,
        ...
        self.form_transform = form_transform
        ...
        if preprocess:
            print("preprocessing")
            preprocess_folder(self.folder, process_name, patch_size, scale=scale)
    ...
    def __getitem__(self, idx):
        ...
        tensor_data = torch.load(path)  # load both patches and control data
        patches = tensor_data["patches"]
        control_data = tensor_data["control_data"]
        ...
        # Apply transforms if specified (with reproducibility)
        ...
        return (
            (center_patch * 2 - 1).to(device()),
            (nbhd_patch * 2 - 1).to(device()),
            (center_ctl * 2 - 1).to(device()),
        )
...
def preprocess_folder(
    folder_path, tensor_path, patch_size=(64, 64), ctlnet=extract_hed_maps, scale=1.0
):
    """
    ...
    for fname in os.listdir(folder_path):
        ...
        control_data = ctlnet(patches_flattened) if ctlnet is not None else None
        ...
        if control_data is not None:
            ...
            control_data = control_data.repeat(1, 1, 3, 1, 1)
