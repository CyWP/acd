...
#     tqdm(range(1), desc="Test")
# except Exception:
#     from tqdm import tqdm

from ..utils.pytorch import const_like
...
EDM2_CONFIGS = {
    "xs": {
        ...
        "url": "https://github.com/CyWP/acd/releases/download/v0.01-pretrained-xs/edm2-img64-xs.pth",
    }
}


def get_edm2(size, folder=".", pretrained=True, force_download=False):
    net = Precond(**EDM2_CONFIGS[size]["config"])
    if pretrained:
        filepath = download_with_progress(
            url=EDM2_CONFIGS[size]["url"], folder=folder, force_download=force_download
        )
        net.load_state_dict(torch.load(filepath, weights_only=False))
    return net
