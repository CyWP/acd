"""
Code mostly taken from https://github.com/nipponjo/deepfillv2-pytorch/blob/master/model/networks.py
"""
...
# ----------------------------------------------------------------------------
DEEPFILL_LINK = (
    "https://github.com/CyWP/pretrained_weights/releases/download/0.0.1/deepfillv2.pth"
)


def get_deepfill(folder, force_download=False):
    if not DeepFillWrapper._instance:
        filepath = download_with_progress(DEEPFILL_LINK, folder, force_download)
        DeepFillWrapper._instance = DeepFillWrapper(filepath)
    return DeepFillWrapper._instance


class DeepFillWrapper(nn.Module):

    _instance = None

    def __init__(self, pretrained_path):
        super().__init__()
        self.model = Generator(checkpoint=pretrained_path)

    def forward(self, img, mask):
        return self.model.infer(img, mask, return_vals=["inpainted"])
...
class Generator(nn.Module):
    ...
    def __init__(
        self, cnum_in=5, cnum_out=3, cnum=48, return_flow=False, checkpoint=None
    ):
        super().__init__()
    ...
    def infer(self, image, mask, return_vals=["inpainted", "stage1"]):
        ...
        for return_val in return_vals:
            if return_val.lower() == "stage1":
                output.append(x_stage1)
            elif return_val.lower() == "stage2":
                output.append(x_stage2)
            elif return_val.lower() == "inpainted":
                output.append(image_compl)
