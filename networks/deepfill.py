...
class GDeConv(nn.Module):
    ...
    #NOTE: Try transposed convolution instead of strict upscaling
...
class Generator(nn.Module):
    ...
    def infer(self, image, mask, sketch=None, return_vals=["inpainted", "stage1"]):
        """
        Args:
        ...
        sketch = (
            sketch if sketch else torch.ones_like(image_masked)[:, :1]
        )  # sketch channel
        x = torch.cat(
            [image_masked, sketch, sketch * mask], dim=1
