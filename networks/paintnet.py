...
class PaintPrecond(PrecondWrapper):
    ...
    def __init__(self, precond):
        ...
        self.label = nn.Parameter(torch.zeros((1000,), dtype=torch.float32))
        self.label_proc = TanhRelu()
    ...
    def forward(self, x, sigma, force_fp32=False, return_logvar=True, **unet_kwargs):
        ...
        labels = self.label_proc(self.label).repeat(ctl.shape[0], 1)
...
class PaintBlock(BlockWrapper):
    ...
    def __init__(self, block, num_convs=2):
